...
describe('propsFor: visibility source', () => {
  let ioCb: ((entries: Partial<IntersectionObserverEntry>[]) => void) | null = null
  ...
  const cross = (target: Element, entry: Partial<IntersectionObserverEntry>) => ioCb!([{ target, ...entry }])

  it('latches once the element is fully contained even if the ratio stalls just below 1', () => {
    const el = document.createElement('div')
    document.body.append(el)

    const dispose = propsFor(el, ['visibility'])
    flushFrames()
    expect(el.style.getPropertyValue('--live-visible')).toBe('0')
    expect(el.style.getPropertyValue('--const-has-entered')).toBe('0')
    ...
    flushFrames()
    expect(el.style.getPropertyValue('--live-visible')).toBe('1')
    expect(el.style.getPropertyValue('--const-has-entered')).toBe('1')
