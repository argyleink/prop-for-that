...
For zero-config, declarative usage, the [`auto` entry](/start/getting-started/#1-auto-zero-config) binds elements straight from a `data-prop` attribute — no `propsFor()` call per element. The functions on this page are for explicit, programmatic control and teardown.
:::
