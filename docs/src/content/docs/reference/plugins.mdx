...
The **import** is camelCase (`scrollVelocity`); the **key** you pass to `propsFor()` / `data-prop` is the dashed form (`'scroll-velocity'`). The tables below list both.
:::
...
Register, then attach with `propsFor(el, [...])`; they write to the bound element. In [auto mode](/start/getting-started/#1-auto-zero-config) a registered element plugin also binds straight from a `data-prop` attribute — just register it before the DOM is scanned:
