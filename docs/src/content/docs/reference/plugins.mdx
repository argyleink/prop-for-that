...
Plugins cover state beyond the core six sources — sensors, network, battery, media, and more. They live in a single tree-shakeable entry, `prop-for-that/plugins`, and you **register** only the ones you use, so nothing you don't import ends up in your bundle.
...
Register several at once with `registerPlugins()` — pass specific sources, or no arguments to register all of them:
...
Registered, then attached with `global([...])` — they write to `:root`.
...
Registered, then attached with `bind(el, [...])` — they write to the bound element.

| Import | Key | Properties |
| --- | --- | --- |
| `elScroll` | `el-scroll` | `--live-scroll-top`, `--live-scroll-left`, `--live-scroll-progress-y` (0–1), `--live-scroll-progress-x` (0–1) |
...
Every plugin **feature-detects** the API it needs and quietly no-ops (returns an empty disposer) when it's unavailable — registering one is always safe.
...
These are deliberately left out — CSS, or the platform, already does them better:
