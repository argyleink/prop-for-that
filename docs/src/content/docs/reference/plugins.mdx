...
An element plugin runs only while its element is in the viewport. Scrolled out of view, its work stops and its last values freeze in place; scrolling back resumes it. So binding `pointer-local`, `video-color`, etc. to many elements costs nothing for the ones nobody can see.
:::
