...
Every `bind()` call returns a disposer. Calling it:
...
- If no sources remain for that element, removes the element from the bindings `WeakMap`.

The `auto` entry also uses a `MutationObserver` to call `unbind` automatically when a `[data-prop]` element is removed from the DOM.
