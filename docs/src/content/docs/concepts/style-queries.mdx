...
- **`@container style()`** applies a **conditional rule** when a property equals a value. Best for **discrete, boolean, or enum** state (`--live-online`, `--live-visible`, `--live-has-entered`, `--live-value: 100`, `--live-paused`, `--live-battery-low`, capability tiers).
